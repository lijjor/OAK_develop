...
def create_depth_pipeline(
    ...
    enable_rgb_depth_align: bool = True,
    align_target: str = "rgb",
    ...
    # 参考 depthai_demo: 仅用于 RGB-Depth 对齐，不做额外输出。
    cam_rgb.setPreviewSize(576, 320)
    ...
    # 与 demo 一致的触发逻辑：LR-check 开启且未禁用时启用对齐。
    # 支持 rgb/left/right 三种对齐目标，便于按场景选择。
    if enable_lrc and enable_rgb_depth_align:
        if align_target == "left":
            stereo.setDepthAlign(dai.CameraBoardSocket.LEFT)
            stereo.setOutputSize(*mono_left.getResolutionSize())
        elif align_target == "right":
            stereo.setDepthAlign(dai.CameraBoardSocket.RIGHT)
            stereo.setOutputSize(*mono_right.getResolutionSize())
        else:
            stereo.setDepthAlign(dai.CameraBoardSocket.RGB)
            stereo.setOutputSize(*cam_rgb.getPreviewSize())
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="400p",
        help="双目相机分辨率，默认 400p（更接近 demo 默认配置）",
    )
    parser.add_argument(
        "--max-display-mm",
        type=int,
    ...
    parser.add_argument(
        "--no-subpixel",
        action="store_true",
        help="关闭亚像素精度（Subpixel，默认开启）",
    )
    parser.add_argument(
        "--subpixel",
        action="store_true",
        help="开启亚像素精度（兼容参数；默认已开启）",
    )
    parser.add_argument(
        "--extended",
        action="store_true",
    ...
    parser.add_argument(
        "--no-rgb-depth-align",
        action="store_true",
        help="禁用 RGB-Depth 对齐；默认行为与 depthai_demo 一致（LR-check 开启时自动对齐到 RGB）",
    )
    parser.add_argument(
        "--align-target",
        choices=["rgb", "left", "right"],
        default="rgb",
        help="对齐目标相机，默认 rgb（可改为 left/right）",
    )
    parser.add_argument(
        "--align",
        action="store_true",
...
def main() -> None:
    ...
    pipeline = create_depth_pipeline(
        ...
        enable_subpixel=(args.subpixel or not args.no_subpixel),
